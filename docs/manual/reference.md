...
| --- | --- | --- |
| "Vision endpoints" ("Vision-Endpunkte") | Ordered list of OpenAI-compatible servers, each with an optional API key (password field beside the address; leave empty for local servers). The plugin pings them in priority order and uses the **first reachable** one. Each entry is a base URL. The settings tab renders a dynamic field per entry; an empty trailing field is the "add new" slot — blurring an empty field removes it from the list. Each field shows its own per-field reachability icon (circle-check / circle-x / loader, with accessible `title` text), and the currently **active** endpoint is marked. See also [Endpoint fallback resolution](#endpoint-fallback-resolution). | `[{"url": "http://localhost:8080"}]` |
...
| "Vision prompt" ("Vision-Prompt") | Text of the **"Default"** prompt preset (large free-text area). The shipped default is itself localized — see note below. Other presets are fixed and chosen via the sidebar preset picker (see the next row). | English: "Transcribe the text in the image exactly to Markdown. Preserve the structure: headings, paragraphs, \*\*emphasis\*\*, lists and tables. Output only the Markdown, no comments." German: "Transkribiere den Text im Bild exakt nach Markdown. Erhalte die Struktur: Überschriften, Absätze, \*\*Hervorhebungen\*\*, Listen und Tabellen. Gib nur das Markdown aus, keine Kommentare." |
| Prompt preset (sidebar dropdown) | Selects which prompt is sent. **Default** uses the editable "Vision prompt" above; the fixed built-ins are **Tables → Markdown**, **Handwriting**, **Math → LaTeX**, **Source code**, **Describe image**. Stored as `promptPreset` in `data.json`; the choice is sticky across runs and sessions. Not a settings-tab control — chosen in the "IMG → MD" sidebar next to the model dropdown. | `default` |
...
- **Per-endpoint API key:** each entry is an object `{ "url": "…", "apiKey": "…" }`; the key is optional and rendered as a password field beside the address. When set, every request to that endpoint — transcription, streaming, ping, model list and the vision-capability probe — carries an `Authorization: Bearer …` header. When unset, no such header is sent at all, which is what local servers expect. This is what lets a local server and a hosted provider share one fallback chain: the key belongs to the endpoint, not to the plugin.
- **Migration:** older `data.json` shapes are carried over on load — both an existing `visionEndpoint` (singular) key and a plain string list `visionEndpoints: ["http://…"]` become `[{ "url": "http://…" }]`. No manual action needed; the old singular key may linger inert in `data.json`, it is not deleted.
