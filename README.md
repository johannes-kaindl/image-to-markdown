...
- **Backlink-based idempotency.** Already-transcribed sources are detected automatically via the vault's backlink index: if a note already carries a `source_pdf` or `source_image` frontmatter field that resolves to the source file, the sidebar shows "exists → open" for that entry instead of re-transcribing. Tick the **"Re-transcribe (overwrite)"** checkbox to force a new transcription; the existing note is overwritten while its full frontmatter (except `transcribed_by`/`pages`) is preserved.
